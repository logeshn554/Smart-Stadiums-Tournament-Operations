...
def test_gate_load_balance_empty():
    """Test 1: Empty gate list returns no recommendations."""
    assert gate_load_balance([]) == []


def test_gate_load_balance_single_gate():
    """Test 2: Single gate cannot trigger load balancing."""
    gate = GateStatus(gate_id="North-A", capacity_percent=90.0, entry_rate=50, wait_time_seconds=300)
    assert gate_load_balance([gate]) == []


def test_gate_load_balance_no_imbalance():
...
def test_gate_load_balance_all_overloaded():
...
def test_gate_load_balance_all_underloaded():
...
def test_gate_load_balance_single_imbalance():
...
def test_gate_load_balance_wait_time_reduction_calculation():
...
def test_gate_load_balance_sorting():
    """Test 8: Recommendations are sorted with the highest wait time reduction first."""
    g_over = GateStatus(gate_id="Gate-Over", capacity_percent=90.0, entry_rate=50, wait_time_seconds=400)
    g_under1 = GateStatus(gate_id="Gate-Under-1", capacity_percent=10.0, entry_rate=2, wait_time_seconds=50)  # Diff: 350 // 2 = 175
    g_under2 = GateStatus(gate_id="Gate-Under-2", capacity_percent=20.0, entry_rate=5, wait_time_seconds=200) # Diff: 200 // 2 = 100
...
def test_gate_load_balance_capping():
    """Test 9: Output is capped to the top 4 (MAX_GATE_RECOMMENDATIONS) pairs."""
    g_over1 = GateStatus(gate_id="Over-1", capacity_percent=90.0, entry_rate=50, wait_time_seconds=500)
    g_over2 = GateStatus(gate_id="Over-2", capacity_percent=85.0, entry_rate=45, wait_time_seconds=400)

    g_under1 = GateStatus(gate_id="Under-1", capacity_percent=10.0, entry_rate=2, wait_time_seconds=20)
    g_under2 = GateStatus(gate_id="Under-2", capacity_percent=15.0, entry_rate=3, wait_time_seconds=30)
    g_under3 = GateStatus(gate_id="Under-3", capacity_percent=20.0, entry_rate=4, wait_time_seconds=40)
...
def test_gate_load_balance_boundary():
...
def test_triage_fire_smoke():
    """Test 11: Fire/smoke incident is Critical and triggers evacuation."""
    inc = IncidentReport(incident_id="INC-1", zone="B2", type="fire_smoke", description="Smoke in zone B2", reporter_role="steward")
...
def test_triage_overcrowding():
    """Test 12: Overcrowding is High severity."""
    inc = IncidentReport(incident_id="INC-2", zone="C1", type="overcrowding", description="Too many fans in C1", reporter_role="steward")
    rec = triage_incident(inc)
    assert rec.severity == SeverityLevel.HIGH
...
def test_triage_medical():
    """Test 13: Medical emergency is High severity."""
    inc = IncidentReport(incident_id="INC-3", zone="A3", type="medical", description="Chest pains", reporter_role="steward")
    rec = triage_incident(inc)
    assert rec.severity == SeverityLevel.HIGH
...
def test_triage_security():
    ...
    inc = IncidentReport(incident_id="INC-4", zone="B4", type="security", description="Fist fight", reporter_role="steward")
    rec = triage_incident(inc)
...
def test_triage_lost_child():
    """Test 15: Lost child incident is Medium severity."""
    inc = IncidentReport(incident_id="INC-5", zone="D2", type="lost_child", description="Boy lost", reporter_role="parent")
    rec = triage_incident(inc)
    assert rec.severity == SeverityLevel.MEDIUM
...
def test_triage_unknown_type():
    """Test 16: Unknown type defaults to Low severity."""
    inc = IncidentReport(incident_id="INC-6", zone="E1", type="spilled_drink", description="Messy spill", reporter_role="cleaner")
    rec = triage_incident(inc)
...
def test_weather_no_hazards():
    """Test 17: No lightning and low heat index triggers nothing."""
    weather = WeatherContext(temperature_celsius=25.0, heat_index=26.0, lightning_detected=False, lightning_radius_km=0.0)
    assert weather_action(weather) == []


def test_weather_lightning_critical():
    """Test 18: Lightning <= 15 km is Critical (shelter in place)."""
    weather = WeatherContext(temperature_celsius=25.0, heat_index=26.0, lightning_detected=True, lightning_radius_km=14.9)
...
def test_weather_lightning_high():
    """Test 19: Lightning > 15 km is High severity (warning only)."""
    weather = WeatherContext(temperature_celsius=25.0, heat_index=26.0, lightning_detected=True, lightning_radius_km=15.1)
    recs = weather_action(weather)
    assert len(recs) == 1
...
def test_weather_extreme_heat():
    ...
    weather = WeatherContext(temperature_celsius=38.0, heat_index=40.0, lightning_detected=False, lightning_radius_km=0.0)
    recs = weather_action(weather)
    assert len(recs) == 1
    assert recs[0].severity == SeverityLevel.HIGH
...
def test_weather_combined_hazards():
    """Test 21: Extreme heat + critical lightning triggers both rules."""
    weather = WeatherContext(temperature_celsius=38.0, heat_index=41.0, lightning_detected=True, lightning_radius_km=10.0)
    recs = weather_action(weather)
    assert len(recs) == 2
...
def test_weather_lightning_boundary():
    """Test 22: Boundary conditions for lightning (exactly 15.0 km) triggers Critical."""
    weather = WeatherContext(temperature_celsius=25.0, heat_index=26.0, lightning_detected=True, lightning_radius_km=15.0)
    recs = weather_action(weather)
    assert recs[0].severity == SeverityLevel.CRITICAL
...
def test_accessibility_fire_evac_available_seats():
    """Test 23: Fire smoke in occupied venue triggers Critical priority assistance."""
    context = EventContext(phase=EventPhase.HALFTIME, total_capacity=50000, occupied_seats=40000, accessible_seats_available=5, concession_queue_avg_minutes=5.0)
    inc = IncidentReport(incident_id="INC-1", zone="B1", type="fire_smoke", description="Kitchen fire", reporter_role="steward")
...
def test_accessibility_overcrowd_available_seats():
    """Test 24: Overcrowding in occupied venue triggers Critical priority assistance."""
    context = EventContext(phase=EventPhase.HALFTIME, total_capacity=50000, occupied_seats=40000, accessible_seats_available=5, concession_queue_avg_minutes=5.0)
    inc = IncidentReport(incident_id="INC-1", zone="B1", type="overcrowding", description="Stair surge", reporter_role="steward")
...
def test_accessibility_full_capacity_seats():
    """Test 25: Zero accessible seats remaining triggers High severity monitor alert."""
    context = EventContext(phase=EventPhase.HALFTIME, total_capacity=50000, occupied_seats=40000, accessible_seats_available=0, concession_queue_avg_minutes=5.0)
    inc = IncidentReport(incident_id="INC-1", zone="B1", type="medical", description="Stubbed toe", reporter_role="steward")
...
def test_accessibility_double_trigger():
    """Test 26: Fire smoke + zero accessible seats triggers BOTH Critical evacuation and High monitor alert."""
    context = EventContext(phase=EventPhase.HALFTIME, total_capacity=50000, occupied_seats=40000, accessible_seats_available=0, concession_queue_avg_minutes=5.0)
    inc = IncidentReport(incident_id="INC-1", zone="B1", type="fire_smoke", description="Fire", reporter_role="steward")
...
def test_accessibility_no_trigger():
    """Test 27: Normal conditions (non-emergency type, accessible seats available) triggers nothing."""
    context = EventContext(phase=EventPhase.HALFTIME, total_capacity=50000, occupied_seats=40000, accessible_seats_available=5, concession_queue_avg_minutes=5.0)
    inc = IncidentReport(incident_id="INC-1", zone="B1", type="medical", description="Dehydration", reporter_role="steward")
...
def test_egress_non_applicable_phase():
...
def test_egress_high_occupancy():
    """Test 29: Post-match with >= 90% occupancy triggers High severity, 3-wave staggered exit."""
    context = EventContext(phase=EventPhase.POST_MATCH, total_capacity=50000, occupied_seats=45000, accessible_seats_available=5, concession_queue_avg_minutes=5.0)
    gates = [
...
def test_egress_moderate_occupancy():
    """Test 30: Overtime with 70-89% occupancy triggers Medium severity, 2-wave staggered exit."""
    context = EventContext(phase=EventPhase.OVERTIME, total_capacity=50000, occupied_seats=38000, accessible_seats_available=5, concession_queue_avg_minutes=5.0)
    gates = [
        GateStatus(gate_id="G1", capacity_percent=20.0, entry_rate=0, wait_time_seconds=30),
        GateStatus(gate_id="G2", capacity_percent=50.0, entry_rate=0, wait_time_seconds=90),
    ]
...
def test_egress_low_occupancy():
    """Test 31: Post-match with < 70% occupancy triggers Low severity standard exit procedures."""
    context = EventContext(phase=EventPhase.POST_MATCH, total_capacity=50000, occupied_seats=30000, accessible_seats_available=5, concession_queue_avg_minutes=5.0)
    gates = [
        GateStatus(gate_id="G1", capacity_percent=20.0, entry_rate=0, wait_time_seconds=30),
        GateStatus(gate_id="G2", capacity_percent=50.0, entry_rate=0, wait_time_seconds=90),
    ]
...
def test_egress_exit_gate_sorting():
...
def test_egress_plan_zero_total_capacity_direct():
